...
export function buildDebugEndpoint(): string {
  return serverConfig.captureAttachUrl ?? `http://127.0.0.1:${serverConfig.captureDebugPort}`;
}
...
export async function launchManagedChrome(startUrl: string): Promise<ChildProcess> {
  const browserPath = await resolveBrowserPath();

  return spawn(
    browserPath,
    [
      `--remote-debugging-port=${serverConfig.captureDebugPort}`,
